...
3. **Mentor é restrito às equipes que mentora** (seção 2 dos requisitos). Admin vê e gerencia todas as equipes; mentor só as vinculadas a ele em `equipe_mentor`; aluno só as que participa. A regra fica em `src/utils/acessoEquipe.ts`. Prazos: o admin altera qualquer um; o mentor, só os das equipes dele.
...
5. **Cadastro de colega só precisa de e-mail + curso, sem RA.** Se o e-mail já existe no sistema, a pessoa é adicionada direto na equipe nova, sem "aceitar convite"; se não existe, a conta já é criada com uma senha provisória **aleatória**, mostrada uma única vez ao líder, e a pessoa é obrigada a trocá-la no primeiro acesso (`usuario.deve_trocar_senha`). Só contas de aluno podem ser vinculadas como colega.
...
8. **Etapas dinâmicas por equipe.** Decisão do InfoHub via WhatsApp: "a jornada padrão segue com 6 etapas, mas o mentor pode acrescentar etapas extras por equipe". `etapa` deixou de ser um catálogo global fixo e passou a pertencer a cada equipe (`etapa.id_equipe`, `etapa.ordem`), com FK composta garantindo que uma equipe nunca aponte pra etapa de outra. Só o mentor **daquela equipe específica** (vínculo em `equipe_mentor`) pode criar etapa extra — nem um mentor de outra equipe, nem um admin sem vínculo. A migração `src/db/migrations/002_etapas_por_equipe.sql` converte bancos que estavam com o schema antigo.
9. **Sessão com refresh token.** O token de acesso (JWT, `JWT_EXPIRES_IN`, padrão 15 min) vai no header `Authorization`. O refresh token (`REFRESH_TOKEN_DIAS`, padrão 7) fica num cookie `httpOnly` restrito a `/api/auth`, e só o hash SHA-256 dele é guardado na tabela `sessao`. Cada `POST /auth/refresh` revoga o token usado e emite outro (rotação); reapresentar um token já revogado encerra todas as sessões do usuário. `POST /auth/logout` revoga a sessão; `PATCH /auth/senha` troca a senha e encerra as outras sessões.
...
login (admin/mentor/aluno), `GET /auth/me`, listagem de equipes com/sem token, as regras de envio de entregável (integrante não envia / líder envia), múltiplos mentores por equipe (incluindo admin virando mentor, e rejeição de aluno como mentor), inscrição pública com auto-vínculo de colega (e-mail novo cria conta, e-mail existente reaproveita), RF-03 completo (criar → logar → desativar → login falha), 404 numa rota inexistente, e erro 422 de validação Zod.
