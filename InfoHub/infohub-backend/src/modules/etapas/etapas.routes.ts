 ...
 */
etapasRouter.get("/", authenticate, requireRole("admin", "mentor"), async (req: Request, res: Response) => {
  const r = await query<{ id_equipe: number }>(`SELECT * FROM etapa ORDER BY id_equipe, ordem`);
  // mentor só recebe as etapas das equipes que mentora
